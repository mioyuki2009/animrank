...
作品映射只使用 `config/titles.json` 中人工确认的 ID。平台未收录或 ID 尚未确认时保存为 `null`，页面显示 `-`。
...
平台缺失时不按 0 分处理，也不进入分子和分母。没有评分或只有一个有效平台时综合分为 `null`，页面显示 `-`；至少两个平台时才进入榜单排序。销量和卷均是商业指标，永远不进入综合分。
